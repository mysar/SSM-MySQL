<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head lang="en">
<meta charset="UTF-8">
<title>注册</title>
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<meta name="viewport"
	content="width=device-width, initial-scale=1.0, minimum-scale=1.0, maximum-scale=1.0, user-scalable=no" />
<meta name="format-detection" content="telephone=no">
<meta name="renderer" content="webkit">
<meta http-equiv="Cache-Control" content="no-siteapp" />

<link rel="stylesheet"
	href="../AmazeUI-2.4.2/assets/css/amazeui.min.css" />
<link href="../css/dlstyle.css" rel="stylesheet" type="text/css">
<script type="text/javascript"
	src="${pageContext.request.contextPath}/js/jquery.min.js"></script>
<script type="text/javascript"
	src="${pageContext.request.contextPath}/js/jquery.easyui.min.js"></script>
<script type="text/javascript"
	src="${pageContext.request.contextPath}/js/locale/easyui-lang-zh_CN.js"></script>
<script src="../AmazeUI-2.4.2/assets/js/jquery.min.js"></script>
<script src="../AmazeUI-2.4.2/assets/js/amazeui.min.js"></script>
<script type="text/javascript"
	src="${pageContext.request.contextPath}/one/layer/layer.js"></script>
<script type="text/javascript"
	src="${pageContext.request.contextPath }/js/Md5.js"></script>
</head>

<body>

	<div class="login-boxtitle">
		<a href="home/demo.html"><img alt="" src="../images/logobig.png" /></a>
	</div>

	<div class="res-banner">
		<div class="res-main">
			<div class="login-banner-bg">
				<span></span><img src="../images/big.jpg" />
			</div>
			<div class="login-box">

				<div class="am-tabs" id="doc-my-tabs">
					<ul class="am-tabs-nav am-nav am-nav-tabs am-nav-justify">
						<li class="am-active"><a href="">用户注册</a></li>
						<li><a href="">手机号注册</a></li>
					</ul>

					<div class="am-tabs-bd">
						<div class="am-tab-panel am-active">
							<form id="ff">

								<div class="user-email">
									<label for="email"><i class="am-icon-envelope-o"></i></label> <input
										type="email" name="email" id="email" placeholder="请输入用户邮箱">
								</div>
								<div class="user-pass">
									<label for="password"><i class="am-icon-lock"></i></label> <input
										type="password" name="" id="password" placeholder="设置密码">
								</div>
								<div class="user-pass">
									<label for="passwordRepeat"><i class="am-icon-lock"></i></label>
									<input type="password" name="" id="password2"
										placeholder="确认密码">
								</div>

							</form>
							<script>
								$(function() {
									
									// 检查用户名
									var checkUserFunc = function() {
										$('#email').blur(function(){
											
										})
										var emailRegx =/^([a-z0-9_\.-]+)@([\da-z\.-]+)\.([a-z\.]{3,6})$/;
										var emails = $('#email').val();
										if (!emailRegx.test(emails)) {
											layer.msg("邮箱格式不正确！", {
												icon : 2
											});
										} else {
											var data = $.ajax({
												type : 'post',
												url : '${pageContext.request.contextPath}/login.do?method=checkEmail',
												data : {
													email : $('#email').val()
												},
												async : false
											});
											if (data.responseJSON.flag == 0) {
												layer.msg("此邮箱可用！", {
													icon : 1
												});
												return true;
											} else {
												layer.msg("该邮箱已被占用！", {
													icon : 2
												});
											}
										}
										return false;
									};
									
									// 检查密码
									var clickPassFunc = function() {
										if ($('#password').val() == '') {
											layer.msg("密码不能为空！", {
												icon : 2
											});
										} else {
											return true;
										}
										return false;
									};
									
									// 检查重复密码
									var clickPass2Func = function() {
										if ($('#password2').val() != $('#password').val()) {
											layer.msg("两次密码不一致！", {
												icon : 2
											});
										} else {
											return true;
										}
										return false;
									};

									$('#email').blur(function() {
										checkUserFunc();
									});

									$('#password').blur(function() {
										clickPassFunc();
									});

									$('#password2').blur(function() {
										clickPass2Func();
									});

									$('#btn').click(function() {
										if (checkUserFunc() && clickPassFunc() && clickPass2Func()) {
											$.post("${pageContext.request.contextPath }/login.do?method=regist", {
												email : $('#email').val(),
												passWord : (hex_md5($('#password').val()))
											}, function(data) {
												if (data.flag == 1) {
													layer.msg('注册成功，前往邮箱激活！', {icon: 1});
													window.location.href="./login.jsp";
												} else {
													layer.msg('注册失败', {icon: 2});
												}
											});
										}
										return false;
									});
								});
								
								
								
								
							</script>

							<div class="login-links">
								<label for="reader-me"> <input id="reader-me"
									type="checkbox" checked="checked"> 点击表示您同意商城《服务协议》
								</label>
							</div>
							<div class="am-cf">
								<input type="button" name="" value="注册"
									class="am-btn am-btn-primary am-btn-sm am-fl" id="btn">
							</div>

						</div>

						<div class="am-tab-panel">
							<form method="post" id="phoneff">
								<div class="user-phone">
									<label for="phone"><i
										class="am-icon-mobile-phone am-icon-md"></i></label> <input type="tel"
										name="phone" id="phone" placeholder="请输入手机号">
								</div>
								<div class="verification">
									<label for="code"><i class="am-icon-code-fork"></i></label> <input
										type="tel" name="" id="code" placeholder="请输入验证码"> <a
										class="btn" href="javascript:void(0);"
										onclick="sendMobileCode();" id="sendMobileCode"> <span
										id="dyMobileButton">获取</span></a>
								</div>
								<div class="user-pass">
									<label for="password"><i class="am-icon-lock"></i></label> <input
										type="password" name="" id="password3" placeholder="设置密码">
								</div>
								<div class="user-pass">
									<label for="passwordRepeat"><i class="am-icon-lock"></i></label>
									<input type="password" name="" id="passwordRepeat"
										placeholder="确认密码">
								</div>
							</form>
							<div class="login-links">
								<label for="reader-me"> <input id="reader-me"
									type="checkbox"> 点击表示您同意商城《服务协议》
								</label>
							</div>
							<div class="am-cf">
								<a
									class="easyui-linkbutton am-btn am-btn-primary am-btn-sm am-fl"
									href="javaScript:submitPF()">注册</a>
							</div>

							<hr>
						</div>

						<script>
							$(function() {
								$('#doc-my-tabs').tabs();
							})
						</script>
						<script type="text/javascript">
					var c="";
					$('#phone').blur(function(){
						var phoneRegx =/^1[3|4|5|7|8][0-9]{9}$/;
						var phone = $('#phone').val();
						if (!phoneRegx.test(phone)) {
							layer.msg("手机号格式不正确！", {
								icon : 2
							});
							return false;
						}else{
							$.post('../../login.do?method=checkPhone',{
								phone:$('#phone').val()
							},
							function(data){
								if(data.flag){
									layer.msg("手机号重复！", {
										icon : 2
									});
									return false;
								}else{
									return true;
								}
							},'json');
						}
					})
				
					function sendMobileCode(){
							$.ajax({
									url:"../../login.do?method=checkPhoneCode",
									type:"post",
									data:{
										phone:$('#phone').val()
									},
									success:function(data){
										if(!data){
											layer.msg("验证码发送失败！", {
												icon : 2
											});
										}
										else	c=data;
									},
									async:false,
									dataType:'text'
							});
					}
					
					// 检查密码
					var clickPassFuncc = function() {
						if ($('#password3').val() == '') {
							layer.msg("密码不能为空！", {
								icon : 2
							});
						} else {
							return true;
						}
						return false;
					};
					// 检查重复密码
					var clickPass2Funcc = function() {
						if ($('#passwordRepeat').val() != $('#password3').val()) {
							layer.msg("两次密码不一致！", {
								icon : 2
							});
						
						} else {
							return true;
						}
						return false;
					};
					$('#password3').blur(function() {
						clickPassFuncc();
					});

					$('#passwordRepeat').blur(function() {
						clickPass2Funcc();
					});
					$('#code').blur(function(){
						if(c!=$('#code').val()){
							layer.msg("验证码输入有误！", {
								icon : 2
							});
						}
					});
					
					function submitPF(){
						if($('#code').val()){
							$.ajax({
							 url:"${pageContext.request.contextPath}/login.do?method=registPhone",
							data:{
								password:(hex_md5($('#password3').val())),
								phone:$('#phone').val()
							},
							success:function(data){
									if (data.flag) {
										layer.msg('注册成功，请前去登录！', {icon: 1});
										window.location.href="./login.jsp";
									} else {
										layer.msg('注册失败', {icon: 2});
									}
							},
							dataType:'json'
						});
						}
						else{
							layer.msg("请输入验证码！", {
								icon : 2
							});
							return false;
						}
					}
						
					</script>
					</div>
				</div>

			</div>
		</div>

		<div class="footer ">
			<div class="footer-hd ">
				<p>
					<a href="# ">恒望科技</a> <b>|</b> <a href="# ">商城首页</a> <b>|</b> <a
						href="# ">支付宝</a> <b>|</b> <a href="# ">物流</a>
				</p>
			</div>
			<div class="footer-bd ">
				<p>
					<a href="# ">关于恒望</a> <a href="# ">合作伙伴</a> <a href="# ">联系我们</a> <a
						href="# ">网站地图</a> <em>© 2015-2025 Hengwang.com 版权所有</em>
				</p>
			</div>
		</div>
</body>

</html>